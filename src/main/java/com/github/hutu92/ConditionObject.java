...
public class ConditionObject {
    ...
    final int fullyRelease(Node node) {
        ...
        try {
            ...
            if (release(savedState)) { // 释放同步状态，也即释放锁
                ...
                return savedState;
            } else { // 如果释放锁失败，则取消等待队列中的该节点，并抛出异常。
     ...
     * <p>
     * 调用{@link #findNodeFromTail}，从尾部向后搜索同步队列来查找该节点
     *
     * @param node the node
    ...
    final boolean isOnSyncQueue(Node node) {
        // 为什么会有下面的这两个条件？
        // 其实我们可以参考ConditionObject.signal()的代码实现。这里的判断就是确保node节点已经正确的从等待队列移到同步队列中。
     ...
     * <p>
     * 如果在发出通知（其它线程调用signal()方法）之前线程被取消（中断），则返回true。
     *
     * @param node the node
     * @return true if cancelled before the node was signalled
     */
    final boolean transferAfterCancelledWait(Node node) {
        // 设置同步状态CAS成功，说明signal方法中的transferForSignal方法还没执行。此后，也不会再执行
        // 所以需要执行signal方法中的enq工作，将当前节点node加入到同步队列中
         ...
         * spin.
         */
        // 设置同步状态CAS失败，说明signal方法中的transferForSignal方法已经在执行了，
        // 需要等待signal方法将当前节点node加入到同步队列中
     ...
     * 检查线程是否中断。
     * <p>
     * 如果在其它线程通知（其它线程调用signal()方法）前中断则返回THROW_IE；
     * 如果在其它线程通知（其它线程调用signal()方法）后之后中断则返回REINTERRUPT；
     * 如果没有中断则返回0；
     */
    ...
    public final void await() throws InterruptedException {
        ...
        // ==================至此，其它线程获取锁后，会在某一时刻调用signal将等待队列中的当前节点移到同步队列尾部==================
        ...
        // 方法acquireQueued会返回是否中断。如果返回true的话，表示中断了，但此时终端标记位已被清除
        if (acquireQueued(node, savedState) && interruptMode != THROW_IE)
            interruptMode = REINTERRUPT;

        // 其它获取锁的线程调用signal将等待队列中的节点移到同步队列尾部这一操作，并不会清除该节点的nextWaiter
        // 所以如果node.nextWaiter不为null，说明等待队列中还有其它等待的节点

        // 等待队列中还有节点，从头遍历，移除等待队列中取消的节点
    ...
    private void doSignal(Node first) { // 参数为等待队列的首节点
        do {
            /*
                ...
                如果移除后，firstWaiter为null，说明等待队列中仅有的一个节点被移除了，lastWaiter也应该置null。
             */
